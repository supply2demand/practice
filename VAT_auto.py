...
filename = '부가세 리스트(박종석)_2023년 2기 예정_231016(실험).xlsx'
cstmr = pd.read_excel(filename,sheet_name= 0)
cst_bsno = cstmr.iloc[2:,6].dropna()
bsno = cst_bsno.tolist()
cst_cmp = cstmr.iloc[2:,5].dropna()
...
options.add_argument(r"--user-data-dir=C:\\Users\\whddl\\AppData\\Local\\Google\\Chrome\\User Data")#로컬 크롬 사용
options.add_argument("--profile-directory=Default")
driver = webdriver.Chrome(options= options)

driver.get("https://www.hometax.go.kr/websquare/websquare.html?w2xPath=/ui/pp/index_pp.xml") 

def HometaxAutoLogin(hometax_id, hometax_pw, crt_pw):
    # 홈택스 페이지로 시작
    ...
    driver.find_element(By.CSS_SELECTOR,"#iptUserId").send_keys(hometax_id) # ID
    driver.find_element(By.CSS_SELECTOR,"#iptUserPw").send_keys(hometax_pw) #비밀번호
    ...
    password = 'azsx5579!!' #인증서와 비밀번호 
    driver.find_element(By.ID, 'input_cert_pw').send_keys(crt_pw)
...
HometaxAutoLogin('HometaxID', 'HometaxPW', '공인인증서 비밀번호')
부가가치세()
print("작업 성공적으로 마침")
driver.close()